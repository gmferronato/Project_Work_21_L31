...
def carica_dataframe(conn):
    df = pd.read_sql("""
        SELECT r.*,
               f.nome,
               f.categoria_fornitura,
               o.tipo_attivita,
               o.zona AS zona
        FROM richieste r
        JOIN fornitori f ON r.id_fornitore = f.id_fornitore
        JOIN operatori o ON r.id_operatore = o.id_operatore
    """, conn)
...
def calcola_kpi(df):
    ...
    return {
        ...
        "tempo_medio_erogazione": round(df_valide["tempo_erogazione"].mean(), 2) if len(df_valide) > 0 else 0.0,
